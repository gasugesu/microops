...
ip_iface_alloc(const char *unicast, const char *netmask)
{
    ...
    iface->broadcast = (iface->unicast & iface->unicast) | ~iface->netmask;
    return iface;
}
...
ip_input(const uint8_t *data, size_t len, struct net_device *dev)
{
    ...
    {
        errorf("fragments dose not support");
        return;
    }
    iface = (struct ip_iface *)net_device_get_iface(dev, NET_IFACE_FAMILY_IP);
    if (!iface)
    {
        // iface is not registerd to the device
        return;
    }
...
ip_output(uint8_t protocol, const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst)
{
    ...
    {
        errorf("unable to ouput with specified souce address, addr=%s", ip_addr_ntop(src, addr, sizeof(addr)));
        return -1;
    }
